function traces_simulation_trajectoire(y_1, y_2, y_3, t_1, t_2, t_3, R_terre, H_c, R_c, V_c)
...
figure();
plot(t(1:size(t_1)), h(1:size(t_1)), 'r');
hold on;
plot(t(size(t_1): size(t_1) + size(t_2)), h(size(t_1):size(t_1) + size(t_2)), 'b');
hold on;
...
figure();
plot(t(1:size(t_1)), V_norm(1:size(t_1)), 'r');
hold on;
plot(t(size(t_1): size(t_1) + size(t_2)), V_norm(size(t_1): size(t_1) + size(t_2)), 'b'); 
hold on;
plot(t(size(t_1) + size(t_2):end), V_norm(size(t_1) + size(t_2):end), 'g');
hold on;
...
figure();
plot(t(1:size(t_1)), M_all(1:size(t_1)), 'r');
hold on;
plot(t(size(t_1): size(t_1) + size(t_2)), M_all(size(t_1): size(t_1) + size(t_2)), 'b'); 
hold on;
plot(t(size(t_1) + size(t_2):end), M_all(size(t_1) + size(t_2):end), 'g');
